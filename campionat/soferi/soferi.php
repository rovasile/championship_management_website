...
$mesajCautare="";
//cele doua campuri pentru filtrarea in functie de nume/echipa sunt in formulare, si deci la executia forumularului verific daca exista ceva trimis prin vectorul $_POST
if ( $_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['nume']!="")
{$queryName =mysqli_real_escape_string($conn, $_POST['nume']);
  $query= "SELECT * FROM soferi S LEFT JOIN echipe E on S.id_echipa = E.id_echipa WHERE nume LIKE '%{$queryName}%' ORDER BY puncte DESC"; //se face o cautare in functie de cuvantul introdus pentru cautare
...
{$queryName =mysqli_real_escape_string($conn, $_POST['nume_echipa']);
  $query= "SELECT * FROM soferi S LEFT JOIN echipe E on S.id_echipa = E.id_echipa WHERE E.nume_echipa LIKE '%{$queryName}%' ORDER BY S.puncte DESC"; //se face o cautare in functie de numele echipei alese pentru cautare
  $mesajCautare="Rezultate pentru ".$queryName.":";
  $_POST = array();
}
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo '<div class="row">';
      ...
      echo '<a href="../profile/profile.php?id='.$row['id_sofer'].'"><div class="sofer">';
        ...
        echo '<p> Data nasterii: '.$row['data_nasterii'].'</p><hr/>';
        echo "<h4> Sponsori:</h4>";

      $querySponsori = "SELECT * FROM soferi S JOIN sponsorizari SP on SP.id_echipa=S.id_echipa WHERE S.id_sofer='".$row['id_sofer']."'";
      //echo $querySponsori;
      $resultSponsori = mysqli_query($conn, $querySponsori);
        if (mysqli_num_rows($resultSponsori) > 0) {
            while($rowSponsori = mysqli_fetch_assoc($resultSponsori))
              echo $rowSponsori['nume_sponsor'].'<br>';
        }
